...
SELECT nacionalidad, edad, COUNT(*) AS cantidad_detenciones
FROM `2022`
GROUP BY nacionalidad, edad
ORDER BY nacionalidad, edad;

SELECT tipo_arma, COUNT(*) AS total_detenciones
FROM `2023`
WHERE tipo_arma IS NOT NULL
GROUP BY tipo_arma
...
SELECT 
    ...
    d.nombre_infraccion AS tipo_infraccion_mas_comun,
    apr.sexo AS sexo_detenido,
    COUNT(apr.sexo) AS detenciones_por_sexo
FROM `2024` apr
LEFT JOIN Delito d ON apr.presunta_infraccion = d.nombre_infraccion
LEFT JOIN Provincia p ON apr.nombre_provincia = p.nombre_provincia
GROUP BY p.nombre_provincia, d.nombre_infraccion, apr.sexo
